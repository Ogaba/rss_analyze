...
_FIC=gnuplot/${_FILE}.log.png
_XMAX=`tail -n1 $_DATA | awk '{ print $2 }'`
_YMAX=`cat $_DATA | awk '{ print $1 }' | sort -g | tail -n1`
_NB=`cat $_DATA | awk '{ print $1 }' | tr '\n' '+' | sed 's/+$/\n/' | bc -l`

# Generate Plot if data exists
>$_TMP
f_set_terminal_png 10000 5000
f_set_margins
echo "set boxwidth 0.1" >> $_TMP
echo "set style fill solid" >> $_TMP
echo "set multiplot" >> $_TMP
echo "set key title \"Fréquence d'apparition d'une distance cumulée entre les mots d'un gros titre, en fonction de la distance cumulée entre les mots de ce gros titre ($_NB phrases analysées)\"" >> $_TMP
f_set_xlabel_nombre_lev_log
f_set_ylabel_nombre_lev_seq_log
echo "set logscale x" >> $_TMP
echo "set logscale y" >> $_TMP
echo "set xrange [1:2000]" >> $_TMP
echo "set xtics (\"1\" 1, \"10\" 10, \"100\" 100)" >> $_TMP
echo "set yrange [1:$(($_YMAX + $_YMAX))]" >> $_TMP
echo "set ytics (\"1\" 1, \"10\" 10, \"100\" 100)" >> $_TMP
#echo "set samples 100" >> $_TMP
echo -n "plot " >> $_TMP
echo " adding ${_DATA} ..."
f_plot_BOXES3 "$_DATA" 1 2
#f_plot_LP3 "$_DATA" 1 2 1 9
